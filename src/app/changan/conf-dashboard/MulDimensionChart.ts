...
export class MulDimensionChart {
  optionData = {
    xAxisData: null,
    yAxisData: null,
    gridData: null,
  ...
  properties = null;
  dom = null;
  colors = ["#4cc5f4", "#fa5879", "#f8e402", "#4feaef", "#9d58fa", "#5899fa", "#58fa6b", "#fa6b58"];
  chart = null;
  constructor(dom,data, dimension, measure) {
    this.finalData = data;
   ...
   */
  groupByData(data, dLen, mLen, groupDimenIndex) {
   ...
   */
  adjustDataOrder(data, dLen, mLen) {
    ...
    for (var i = 0; i < visMLen; i++) {
      ...
      newData[i] = data.map(function (dataItem) {
        ...
        len = len - visMLen - 1;
        while (len) {
          newDataItem.push({value:dataItem[len - 1],name:that.properties.dimension[len-1].alias});
          len --;
        }
        return newDataItem;
      });
   ...
   */
  getDimensionCategory(data, dLen, mLen, isContinue) {
    var chartData = [];
    var subTypeData = [];
    var axisLabelData = [];
    var chartLabelData = [];
    ...
    const peakValue = processMeasureData(chartData, dLen, mLen);
    return {
      chartData: chartData,
   ...
   */
  getPercentData(chartLabelData, isContinue) {
    var categoryLabel = [];
   ...
   */
  standarHigherData (data, measure, dimension) {
    ...
    function getDimensionPeak(chartData, dimension, index, context) {
      ...
      var dateFormat = key.timeFormat || key.colType;
  ...
  updateAxis (axisModel, axis, key, chartType, contentWidth) {
   ...
   * @param {Object} axis
   * @param {Object} key
   * @param {Object} contentWidth
   */
  updateLabelFormatter (type, timeFormat, contentWidth) {
    if (type == 'time') {
    ...
    function getDimensions(data) {
      ...
      if (data && data[0]) {
        for (var i = 0; i < data[0].length; i++) {
          ...
          i > 1 && dimensions.push( {type: 'ordinal'});
        }
      }
      return dimensions;
    }
  };
  joinFeature(series, state){
    var that = this;
   ...
   */
  sortSeriesData(type, data, dateFormat, context) {
   ...
   */
  preprocessCategory(data, realChartWidth, continuousKey, chartData) {
   ...
   */
  processTooltipFormatter(dimension, measure, disContinuousKey, params) {
    console.log(params)
    ...
    while (len - 2) {
      formatter += getFormatter(color, data[len - 1].name, data[len - 1].value);
      len -- ;
      aliasIndex ++ ;
    }
    if (params.componentType == 'markLine') {//处理参考线的显示悬浮层
      ...
      formatter += getFormatter(color, data[0].name, timeFormat ? new Date(data[0]).Format(timeFormat) : data[0]);
      measure.length > 0 && (formatter += getFormatter(color, params.seriesName, data[1]));
    }
    return formatter;
    /**
     获取数据的html字符串
  ...
  /**
   ...
   @return {Array} disGroupIndex: 无需分组的序号
   */
  getDisGroupItem(dLen, groupDimenIndex) {
    var disGroupIndex = [];
    for (var i = 0; i < dLen; i++) {
      if (groupDimenIndex.indexOf(i) == -1) {
        disGroupIndex.push(i);
