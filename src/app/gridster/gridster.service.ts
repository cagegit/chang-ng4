...
export class GridsterService {
    ...
    scaleItemSize(el:HTMLElement,moveX:number,moveY:number){
      console.log(moveX,moveY);
      let item = this.getItemByElement(el);
      el.style.width = this.getItemWidth(item) + moveX + 'px';
      el.style.height = this.getItemHeight(item) + moveY +'px';
      if(moveX > 0){
        item.w = moveX > this._cellWidth/2 ? item.w + 1 : item.w;
      }else {
        item.w = Math.abs(moveX) > this._cellWidth/2 ? item.w - 1 : item.w;
      }
      if(moveY > 0){
       item.h = moveY > this._cellWidth/2 ? item.h + 1 : item.h;
      }else {
       item.h = Math.abs(moveX) > this._cellHeight/2 ? item.h - 1 : item.h;
      }
      // item.w=this.getItemWidth(item) + moveX;
      // item.h=this.getItemHeight(item) + moveY ;
       this.highlightPositionForItem(item);
      console.log(item);
    }
    //getSpecificItem(el:HTMLElement) {
    ...
    private calculateCellSize () {
      ...
      console.log('width',this._cellWidth);
      console.log("height",this._cellHeight);
    }
  ...
  getItemWidth (item) {
    ...
    private setElementHeight(){
      let maxHeightList = this.items.map((item) =>{
        return item.h +item.y;
      })
      let maxHeight = Math.max.apply(null,maxHeightList);
      let parentNode = <HTMLElement>this.$element.parentNode;
      parentNode.style.height = this._cellHeight * maxHeight +'px';
    }
