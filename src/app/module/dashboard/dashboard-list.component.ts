...
export class DashboardListComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute,private router: Router,public authService : AuthService,private dashboardService:DashboardService,private appNotification : AppNotification,private appWebSocketService:AppWebSocketService,private appContext:AppContext,
              private dataHandleSer: DataHandleService,private dataSetService:DataSetService){
  }

  ngOnInit(){
    this.route.params.subscribe((params) => {
      console.info("初始化查询参数:",params);
      this.filterDashboard.createUserDisplayName = params['createUserDisplayName'];
    ...
    this.dataHandleSer.getDashboards().subscribe((list)=>{
  ...
  toCreateDashboard(){
    this.dataSetService.getSetList('','').subscribe((response)=>{
  ...
  get dashboardList(){
    if(this._dashboardList&& Array.isArray(this._dashboardList)) {
      return this._dashboardList.filter((temp)=> {
        ...
        return (this.filterDashboard.dashboardName? new RegExp(this.filterDashboard.dashboardName.toLowerCase()).test(temp.dashboardName.toLowerCase()) : true)&&
  ...
  delete(dashboardID){
    this.dataHandleSer.deleteDashboard(dashboardID).subscribe((b:boolean)=>{
      if(b) {
