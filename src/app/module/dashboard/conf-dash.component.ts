...
export class ConfDashComponent implements AfterViewInit,OnInit {
  ...
  set dataSetID(id:string) {
    ...
    this.changSer.getTablesByMenuId(id).subscribe(res => {
      if(res && res.data && Array.isArray(res.data.tableList)) {
        ...
        //   let thinQuery = JSON.parse(this.cardList[0].content.replace(/\n/g, ' ')) as query.QueryTemplate;
  ...
  menuList = [];
  tableList = [];
  constructor(private route:ActivatedRoute ,private renderer:Renderer2,private dashboardService:DashboardService,private cardService:CardService,private router:Router,private appNotification:AppNotification,private dragulaService: DragulaService,private sanitizer: DomSanitizer,
              private changSer:ChangService,private dataHandleSer: DataHandleService){
  ...
  changeFolderState(e:any) {
    ...
    //   b = true;
    // }
    this.renderer.addClass(target, 'grade_fold');
  }
  ...
  changePage(page:Page) {
    ...
    //   if(this.usedCards.indexOf(card.cardId)>=0){
    //     card['hasUsed']=true;
    //   }
    // })
    // this.curPage.pageFilters.forEach(filter=>{
    //   this.loadDefaultPageFilterData(filter);
    // })
  }
  ...
  tabClick(page:Page,e:MouseEvent){
    ...
    this.dbClickTimer=setTimeout(()=>{
      if(this.count==1){
        ...
        this.changeTab(page);
        let target = <any>e.target;
        this.renderer.addClass(target.parentNode, 'cur');
      }
      if(this.count==2){
        console.log("双击222");
  ...
  createPage() {
    ...
    p.dataSetID = this.menuList[0].id;
  ...
  showPageNameInputBox(e:MouseEvent) {
    ...
    let width = target.getBoundingClientRect().width;
    let inputNode = target.parentNode.getElementsByTagName('input')[0];
    this.renderer.setStyle(inputNode, 'width', `${width}px`);
    this.renderer.addClass(target.parentNode, 'input_state');
    inputNode.focus();
    inputNode.select();
  }
  ...
  clearPanel(panel:Panel) {
    ...
    if (panel.panelType == 0 && data && panel.data.cardId) {
  ...
  showPanelInputBox(e:MouseEvent) {
    ...
    // this.renderer.setElementStyle(inputNode, 'width', `${width}px`);
    this.renderer.setStyle(inputNode, 'display', 'block');
    inputNode.focus();
    inputNode.select();
  }
